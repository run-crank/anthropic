...
describe('CompletionSemanticSimilarity', () => {
  ...
  describe('Metadata', () => {
    it('should return expected step metadata', () => {
      ...
      expect(stepDef.getName()).to.equal('Check Anthropic semantic similarity of response to provided text from completion');
      expect(stepDef.getExpression()).to.equal(`Anthropic model (?<model>[a-zA-Z0-9_ -.]+) response to "(?<prompt>[a-zA-Z0-9_ -'".,?!]+)" semantically compared with "(?<comparetext>[a-zA-Z0-9_ -'".,?!]+)" should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<semanticsimilarity>.+)?`);
  ...
  describe('ExecuteStep', () => {
    describe('Anthropic response meets expected semantic similarity', () => {
      beforeEach(() => {
        // Setup for the scenario where the Anthropic response is semantically similar to the expected text
    ...
    describe('Anthropic response does not meet expected semantic similarity', () => {
      beforeEach(() => {
        // Setup for the scenario where the Anthropic response is not semantically similar to the expected text
        const expectedModel: string = 'claude-model';
        const expectedPrompt: string = 'Hello, Anthropic!';
        const compareText: string = 'Goodbye, Anthropic!';
        protoStep.setData(Struct.fromJavaScript({
          model: expectedModel,
          prompt: expectedPrompt,
          comparetext: compareText,
    ...
    describe('Error occurred while fetching Anthropic response', () => {
      beforeEach(() => {
