...
describe('CompletionTokenCost', () => {
  ...
  describe('Metadata', () => {
    it('should return expected step metadata', () => {
      ...
      expect(stepDef.getName()).to.equal('Check Anthropic prompt token cost given a prompt and model');
      expect(stepDef.getExpression()).to.equal('Anthropic model (?<model>[a-zA-Z0-9_-]+) ?(?<type>.+)? token cost in response to "(?<prompt>[a-zA-Z0-9_ -]+)" should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<expectation>.+)? tokens');
  ...
  describe('ExecuteStep', () => {
    describe('Anthropic prompt response meets token cost expectation', () => {
      beforeEach(() => {
        // Setup for the scenario where the Anthropic response meets the expected token cost
    ...
    describe('Anthropic prompt response does not meet token cost expectation', () => {
      beforeEach(() => {
        // Setup for the scenario where the Anthropic response does not meet the expected token cost
        const expectedModel: string = 'claude-model';
