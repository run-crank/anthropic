...
describe('CompletionEqualsAb', () => {
  ...
  describe('Metadata', () => {
    it('should return expected step metadata', () => {
      ...
      expect(stepDef.getName()).to.equal('Compare Anthropic model A and B prompt responses from completion');
      expect(stepDef.getExpression()).to.equal(`Anthropic model (?<modela>[a-zA-Z0-9_ -.]+) and (?<modelb>[a-zA-Z0-9_ -.]+) responses to "(?<prompt>[a-zA-Z0-9_ -'".,?!]+)" should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<expectation>.+)?`);
  ...
  describe('ExecuteStep', () => {
    describe('Anthropic models A and B prompt responses meet expectation', () => {
      beforeEach(() => {
        ...
        clientWrapperStub.getChatCompletion.withArgs('claude-model-a').returns(Promise.resolve({
          choices: [{ message: { content: 'expected response' } }],
        }));
        clientWrapperStub.getChatCompletion.withArgs('claude-model-b').returns(Promise.resolve({
          choices: [{ message: { content: 'expected response' } }],
        }));
      });
    ...
    describe('Anthropic models A and B prompt responses do not meet expectation', () => {
      beforeEach(() => {
