...
describe('CompletionEquals', () => {
  ...
  describe('Metadata', () => {
    it('should return expected step metadata', () => {
      ...
      expect(stepDef.getName()).to.equal('Check Anthropic prompt response from completion');
      expect(stepDef.getExpression()).to.equal(`Anthropic model (?<model>[a-zA-Z0-9_ -.]+) response to "(?<prompt>[a-zA-Z0-9_ -'".,?!]+)" should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<expectation>.+)?`);
  ...
  describe('ExecuteStep', () => {
    describe('Anthropic prompt response meets expectation', () => {
      beforeEach(() => {
    ...
    describe('Anthropic prompt response does not meet expectation', () => {
      beforeEach(() => {
        const expectedModel: string = 'claude-model';
        const expectedPrompt: string = 'Hello, Anthropic!';
        protoStep.setData(Struct.fromJavaScript({
