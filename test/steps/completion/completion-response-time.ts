...
describe('CompletionResponseTime', () => {
  ...
  describe('Metadata', () => {
    it('should return expected step metadata', () => {
      ...
      expect(stepDef.getName()).to.equal('Check Anthropic prompt response response time from request to completion in milliseconds');
      expect(stepDef.getExpression()).to.equal('Anthropic model (?<model>[a-zA-Z0-9_-]+) response time in response to "(?<prompt>[a-zA-Z0-9_ -]+)" should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<expectation>.+)? ms');
  ...
  describe('ExecuteStep', () => {
    describe('Anthropic prompt response meets response time expectation', () => {
      beforeEach(() => {
        // Setup for the scenario where the Anthropic response meets the expected response time
    ...
    describe('Anthropic prompt response does not meet response time expectation', () => {
      beforeEach(() => {
        // Setup for the scenario where the Anthropic response does not meet the expected response time
        const expectedModel: string = 'claude-model';
        const expectedPrompt: string = 'Hello, Anthropic!';
        protoStep.setData(Struct.fromJavaScript({
          model: expectedModel,
